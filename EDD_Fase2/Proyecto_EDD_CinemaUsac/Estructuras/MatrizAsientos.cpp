...
MatrizAsientos::~MatrizAsientos() {
    // Liberar cada asiento y cada cabecera
    if (raiz == nullptr) return;
    NodoMatriz* fila = raiz->abajo;
    while (fila != nullptr) {
        NodoMatriz* sigFila = fila->abajo;
        NodoMatriz* asiento = fila->derecha;
        while (asiento != nullptr) {
            NodoMatriz* sig = asiento->derecha;
            delete asiento;
            asiento = sig;
        }
        delete fila;
...
NodoMatriz* MatrizAsientos::crearFila(int fila) {
    ...
    while (actual->abajo != nullptr && actual->abajo->fila < fila) actual = actual->abajo;
...
NodoMatriz* MatrizAsientos::crearColumna(int columna) {
    ...
    NodoMatriz* actual = raiz;
    while (actual->derecha != nullptr && actual->derecha->columna < columna) actual = actual->derecha;
    nuevaColumna->derecha = actual->derecha;
    if (actual->derecha != nullptr) actual->derecha->izquierda = nuevaColumna;
...
bool MatrizAsientos::insertarAsiento(int fila, int columna, std::string codigoReserva) {
    ...
    if (cabeceraFila == nullptr) cabeceraFila = crearFila(fila);

    NodoMatriz* cabeceraCol = buscarColumna(columna);
    if (cabeceraCol == nullptr) cabeceraCol = crearColumna(columna);

    NodoMatriz* nuevoAsiento = new NodoMatriz(fila, columna, "Ocupado");
    nuevoAsiento->codigoReserva = codigoReserva;
    ...
    if (actualFila->derecha != nullptr) actualFila->derecha->izquierda = nuevoAsiento;
...
int MatrizAsientos::getFilas() const { return filas; }
int MatrizAsientos::getColumnas() const { return columnas; }
std::string MatrizAsientos::getPelicula() const { return pelicula; }
std::string MatrizAsientos::getHorario() const { return horario; }
std::string MatrizAsientos::getSala() const { return sala; }
NodoMatriz* MatrizAsientos::getRaiz() { return raiz; }

std::vector<std::tuple<int, int, std::string>> MatrizAsientos::obtenerReservas() const {
    std::vector<std::tuple<int, int, std::string>> reservas;
    if (raiz == nullptr) return reservas;
    NodoMatriz* fila = raiz->abajo;
    while (fila != nullptr) {
        NodoMatriz* asiento = fila->derecha;
        while (asiento != nullptr) {
            if (!asiento->codigoReserva.empty())
                reservas.emplace_back(asiento->fila, asiento->columna, asiento->codigoReserva);
...
bool MatrizAsientos::buscarPorCodigoReserva(const std::string& codigoReserva, int& filaOut, int& colOut) const {
    if (raiz == nullptr) return false;
    NodoMatriz* fila = raiz->abajo;
    while (fila != nullptr) {
        NodoMatriz* asiento = fila->derecha;
        while (asiento != nullptr) {
            if (asiento->codigoReserva == codigoReserva) {
                filaOut = asiento->fila;
                colOut = asiento->columna;
                return true;
            }
            asiento = asiento->derecha;
        }
